...
(function (window) {
  ...
  // ============================================================
  // WikiMini - Focused Article View with Navigation
  // ============================================================
  M.UI.WikiMini = (props = {}) => {
    const db = props.db || {};
    ...
    if (!article) {
      return D.Containers.Div({ attrs: { class: 'p-8 text-center text-[var(--muted-foreground)]' } }, [
        'Select a wiki article'
      ]);
    }

    const title = article.title ? article.title[lang] : article.id;
    const content = article.content ? article.content[lang] : '';
    const renderedHtml = window.marked ? window.marked.parse(content) : content;

    return D.Containers.Div({ attrs: { class: 'flex flex-col h-full bg-[var(--background)]' } }, [
      // Header with back button
      D.Containers.Div({ attrs: { class: 'border-b border-[var(--border)] p-4 flex items-center gap-3 bg-[var(--card)]' } }, [
        onBack ? M.UI.Button({
          variant: 'ghost',
          size: 'sm',
          attrs: {
        ...
        D.Text.H2({ attrs: { class: 'text-xl font-bold flex-1 text-[var(--foreground)]' } }, [title])
      ]),

      // Content area
      D.Containers.Div({ attrs: { class: 'flex-1 overflow-auto p-6' } }, [
        D.Containers.Div({
          attrs: {
